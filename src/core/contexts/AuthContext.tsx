...
import { localAuthService, LocalUser } from '@/features/auth/services/localAuth';
...
export const AuthProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const signIn = async (email: string, password: string) => {
    try {
      // Validar entrada
      if (!email || !password) {
        return { data: null, error: new Error('Por favor, preencha todos os campos') };
      }
      
      // Validar formato de e-mail
      const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
      if (!emailRegex.test(email)) {
        return { data: null, error: new Error('Por favor, insira um e-mail válido') };
      }
      ...
      if (result) {
        // Login bem-sucedido
        setUser(result);
        // Emitir um evento customizado para notificar sobre a mudança de autenticação
        window.dispatchEvent(new CustomEvent('authStateChanged', { detail: { isAuthenticated: true } }));
        return { data: { user: result }, error: null };
      } else {
        // Credenciais inválidas - mensagem genérica para segurança
        return { data: null, error: new Error('Credenciais inválidas. Por favor, verifique seu e-mail e senha.') };
      }
    } catch (error: any) {
      // Erro no login - mensagem genérica para segurança
      const appError = handleError(error, 'auth', false); // Não mostrar toast aqui, pois será tratado no componente
      return { data: null, error: new Error('Ocorreu um erro durante o login. Por favor, tente novamente.') };
    }
  };
