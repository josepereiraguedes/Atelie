...
import { useLocalDatabase, PurchaseQuoteItem as PurchaseQuoteItemInterface } from '../../contexts/LocalDatabaseContext';
...
type PurchaseQuoteItem = PurchaseQuoteItemInterface;
...
const PurchaseQuoteForm: React.FC = memo(() => {
  const { id } = useParams();
  ...
  const handleItemChange = useCallback((index: number, field: keyof PurchaseQuoteItem, value: string | number) => {
    ...
    try {
      await handleFormSubmit(async () => {
        const quoteData = {
          ...
          validity_date: formData.expected_delivery, // Corrigido para usar validity_date
          status: formData.status,
          notes: formData.notes,
          total: calculateTotal(),
          items: formData.items // Incluir os itens no envio
        };
  ...
  }, [formData, id, handleFormSubmit, updatePurchaseQuote, addPurchaseQuote, goTo, calculateTotal]);
