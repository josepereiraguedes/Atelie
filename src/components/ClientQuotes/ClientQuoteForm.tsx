...
import { useLocalDatabase, ClientQuote, Product, ClientQuoteItem } from '../../contexts/LocalDatabaseContext';
...
const ClientQuoteForm: React.FC = () => {
  ...
  const {
    ...
    calculateTotal: calculateQuoteTotal,
    updateItemProduct
  ...
  const handleItemChange = useCallback((index: number, field: keyof ClientQuoteItemForm, value: string | number) => {
    ...
    if (field === 'product_id') {
      ...
      if (product) {
        updateItemProduct(index, value as number, product.sale_price || 0);
      } else {
        updateItemProduct(index, value as number);
      }
    } else {
  ...
  }, [products, updateItemProduct, updateQuoteItem, quoteItems]);
