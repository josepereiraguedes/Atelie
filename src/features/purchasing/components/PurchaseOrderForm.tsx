...
import { Save, X, Plus, Trash2, Package, AlertCircle } from 'lucide-react';
import { useLocalDatabase, Product } from '@/core/contexts/LocalDatabaseContext';
...
const PurchaseOrderForm: React.FC = () => {
  ...
  console.log('Renderizando PurchaseOrderForm com id:', id);
  ...
  const handleItemChange = useCallback((index: number, field: keyof PurchaseOrderItem, value: string | number) => {
    // Usar o hook useFormItems para atualizar o item
    updateOrderItem(index, field, value);
    
    // Atualizar o formData para manter a sincronização
    setFormData(prev => ({
      ...prev,
  ...
  const handleProductSelect = useCallback((index: number, productId: number) => {
    ...
    if (product) {
      ...
      updateOrderItem(index, 'unit_price', product.cost || 0);
      updateOrderItem(index, 'total', (orderItems[index]?.quantity || 0) * (product.cost || 0));
    } else {
      updateOrderItem(index, 'product_id', productId);
    }
  ...
  }, [products, updateOrderItem, orderItems]);
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <select
                ...
                required
                className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white ${formData.supplier_id === 0 ? 'border-red-500' : 'border-gray-300'}`}
              >
            ...
            <div>
              ...
              <input
                ...
                onChange={handleChange}
                required
                className={`w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white dark:bg-gray-700 text-gray-900 dark:text-white ${!formData.order_date ? 'border-red-500' : 'border-gray-300'}`}
              />
              {!formData.order_date && (
                <div className="mt-1 flex items-center text-sm text-red-600">
                  <AlertCircle className="w-4 h-4 mr-1" />
