...
const SalesChart: React.FC<SalesChartProps> = memo(({ salesData }) => {
  // Se não houver dados suficientes, exibir mensagem
  if (!salesData || salesData.length === 0) {
    return (
      <div className="bg-white dark:bg-gray-800 rounded-xl shadow-sm p-6 flex flex-col items-center justify-center h-[300px]">
        <h3 className="text-lg font-medium text-gray-900 dark:text-white mb-2">
          Tendência de Vendas e Compras
        </h3>
        <p className="text-gray-500 dark:text-gray-400">Sem dados suficientes para exibir o gráfico</p>
      </div>
    );
  }
  ...
  // Calcular o denominador para a escala X, evitando divisão por zero
  const xDenominator = salesData.length > 1 ? salesData.length - 1 : 1;

  // Preparar pontos para o gráfico de vendas
  const salesPoints = salesData.map((d, i) => {
    const x = padding + (i * (width - 2 * padding) / xDenominator);
