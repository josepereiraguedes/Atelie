      ...
      const salesByProduct = sales.reduce((acc, sale) => {
        ...
        if (sale.items && sale.items.length > 0) {
          sale.items.forEach(item => {
            ...
            acc[item.product_id].revenue += item.total;
        ...
        // Para transações com item único (compatibilidade com versão anterior)
          ...
          const saleCost = (sale.quantity || 0) * product.cost;
              ...
              {(() => {
                ...
                return detailedTransactions.map((transaction, index) => (
                  <tr key={index} className="border-b border-gray-100 dark:border-gray-700/50 last:border-0">
                    ...
                    <td className="py-2 text-gray-900 dark:text-white">R$ {transaction.unit_price.toFixed(2)}</td>
                    ...
                    <td className="py-2 text-red-600 font-medium">R$ {transaction.cost.toFixed(2)}</td>
                    <td className={`py-2 font-medium ${transaction.profit >= 0 ? 'text-green-600' : 'text-red-600'}`}>
